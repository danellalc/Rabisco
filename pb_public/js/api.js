...
export function createApi({ getToken, getUserId, onSession = () => {} }) {
  ...
  return {
    ...
    listTrash: () => fresh('GET', '/api/trash'),
    trash: (kind, id) => fresh('POST', '/api/trash', { body: { kind, id } }),
    restore: (kind, id) => fresh('POST', '/api/trash/restore', { body: { kind, id } }),
    emptyTrash: () => fresh('DELETE', '/api/trash'),
